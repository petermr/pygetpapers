...
class PygetpapersUI:
    ...
    def render_sidebar(self):
        ...
        page = st.sidebar.selectbox(
            "Choose a page:",
            ["Search Papers", "Query Builder", "Corpus Manager", "Data Tables", "Figures Gallery", "Corpus Comparison", "Fulltext Search", "Settings", "Help"]
        )
    ...
    def _render_corpus_datatables(self, corpus_name: str):
        ...
        try:
            ...
            tab1, tab2, tab3, tab4, tab5 = st.tabs(["📄 Papers", "📋 Metadata", "📊 Summary", "🔍 Fulltext Search", "💾 Export"])
                        ...
                        if "search_results" in st.session_state and st.session_state.search_results:
                            ...
                            if st.button("📋 Show Only Papers with Matches"):
                                filtered_data = self.datatables.filter_papers_by_search(
                                    st.session_state.current_corpus_data, 
                                    results
                                )
                ...
                with st.spinner("Extracting figures and captions..."):
                    try:
                        ...
                        if figures_data["papers"]:
                            ...
                            st.markdown("---")
                            st.markdown("**For more detailed figure analysis, visit the [Figures Gallery](#figures-gallery) page.**")
                        else:
    ...
    def render_figures_gallery(self):
        ...
        if selected_corpus:
            ...
            if figures_data["papers"]:
                ...
                with col3:
                    st.metric("Average per Paper", f"{summary['total_figures'] / summary['papers_with_figures']:.1f}" if summary['papers_with_figures'] > 0 else "0")
                with col4:
                ...
                with tab2:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        figure_types = list(set(fig["figure_type"] for paper_figs in figures_data["papers"].values() for fig in paper_figs["figures"]))
                ...
                with st.expander(f"🔄 Rerun: {corpus['name']}"):
                    ...
                    with col2:
                        ...
                        if st.button(f"🔄 Rerun {corpus['name']}", key=f"rerun_{i}"):
                            st.info(f"Rerun functionality will be implemented to update {corpus['name']} with {new_limit} papers")
                    ...
                    if results["matches"]:
                        ...
                        with col1:
                            if st.button("📋 Filter Papers by Matches"):
                                filtered_data = self.datatables.filter_papers_by_search(
                                    st.session_state.fulltext_corpus_data, 
                                    results
                                )
                        ...
                        with col3:
                            if st.button("📊 Show Match Distribution"):
                                ...
                                if match_counts:
                                    ...
                                    fig = px.histogram(df_matches, x="Matches", 
