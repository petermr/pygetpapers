...
class BioRxivWebScraper:
    ...
    def _parse_search_results(self, html_content: str, max_results: int) -> Dict[str, Any]:
        ...
        if results_list:
            ...
            paper_elements = results_list.find_all('li', class_=re.compile(r'search-result'))
        ...
        try:
            ...
            if title_elem:
                ...
                if title_link:
                    ...
                    paper_data['url'] = urljoin(self.base_url, title_link.get('href', ''))
            
            # Extract authors
            authors_elem = citation_elem.find('div', class_='highwire-cite-authors')
            if authors_elem:
                author_spans = authors_elem.find_all('span', class_='highwire-citation-author')
                authors = []
            ...
            if metadata_elem:
                # Extract journal
                journal_elem = metadata_elem.find('span', class_='highwire-cite-metadata-journal')
                if journal_elem:
                    paper_data['journal'] = journal_elem.get_text(strip=True)
                
                # Extract bioRxiv ID
                pages_elem = metadata_elem.find('span', class_='highwire-cite-metadata-pages')
                if pages_elem:
                    paper_data['biorxiv_id'] = pages_elem.get_text(strip=True).rstrip(';')
                
                # Extract DOI
                doi_elem = metadata_elem.find('span', class_='highwire-cite-metadata-doi')
                if doi_elem:
        ...
        if pager_elem:
            ...
            if prev_elem:
                ...
                if prev_link:
                    ...
                    pagination['previous_url'] = urljoin(self.base_url, prev_link.get('href', ''))
