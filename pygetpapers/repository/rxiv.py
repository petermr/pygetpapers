...
class Rxiv(RepositoryInterface):
    ...
    def rxiv(
        ...
        json_metadata_dictionary = self.download_tools._make_dict_from_list(total_result_list, paper_key=DOI)
        for paper in json_metadata_dictionary:
            self.download_tools._add_download_status_keys(paper, json_metadata_dictionary)
    ...
    def make_request_add_papers(self, interval, cursor_mark, source, total_number_of_results, total_papers_list):
    ...
    def make_request_url_for_rxiv(self, cursor_mark, interval, source):
        if type(interval) == int:
            self.get_url = "https://api.biorxiv.org/details/{source}/{interval}".format(source=source, interval=interval)
        else:
            self.get_url = self.download_tools.query_url.format(source=source, interval=interval, cursor=cursor_mark)
    ...
    def download_and_save_results(
        ...
        self.download_tools._make_metadata_json_files_for_paper(
            result_dict[NEW_RESULTS], updated_dict=result_dict[UPDATED_DICT], paper_key=DOI, name_of_file=RXIV_RESULT
        )

    def make_xml_for_rxiv(self, dict_of_papers, xml_identifier, paper_id_identifier, filename):
        ...
        for paper in tqdm(dict_of_papers):
            ...
            path_to_save_xml = os.path.join(str(os.getcwd()), url_encoded_doi_of_paper, filename)
            self.download_tools.queries_the_url_and_writes_response_to_destination(xml_url, path_to_save_xml)
