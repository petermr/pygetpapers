...
class JATS4RConverter:
    """
    JATS4R XSLT converter for JATS XML to HTML conversion.
    """
    ...
    def __init__(self, jats4r_path: Optional[str] = None):
        ...
        Args:
            jats4r_path: Path to JATS4R installation. If None, will download automatically.
        """
    ...
    def _get_jats4r_path(self) -> str:
        """Get or create JATS4R installation path."""
        # Use a local directory for JATS4R
    ...
    def _setup_jats4r(self):
        """Download and setup JATS4R if not already present."""
        jats4r_dir = Path(self.jats4r_path)

        # Check if JATS4R is already set up
        xslt_file = jats4r_dir / "xslt" / "jats2html.xsl"
        if xslt_file.exists():
            self.xslt_path = str(xslt_file)
            self.css_path = str(jats4r_dir / "css" / "jats4r.css")
            logger.info(f"JATS4R found at {self.jats4r_path}")
            return

        # Download JATS4R from GitHub
        logger.info("Downloading JATS4R...")
        self._download_jats4r()

    def _download_jats4r(self):
        """Download JATS4R from GitHub releases."""
        try:
            # Download latest release from GitHub
            url = "https://github.com/PeerJ/jats4r/archive/refs/heads/main.zip"
            ...
            extracted_dir = Path(self.jats4r_path) / "jats4r-main"
            if extracted_dir.exists():
                # Move all contents to parent directory
                for item in extracted_dir.iterdir():
                    shutil.move(str(item), str(Path(self.jats4r_path) / item.name))
            ...
            # Set paths
            self.xslt_path = str(Path(self.jats4r_path) / "xslt" / "jats2html.xsl")
            self.css_path = str(Path(self.jats4r_path) / "css" / "jats4r.css")

            logger.info("JATS4R downloaded and set up successfully")

        except Exception as e:
            logger.error(f"Failed to download JATS4R: {e}")
            raise
    ...
    def convert_corpus_xml_files(
        ...
        try:
            ...
            for xml_file in xml_files:
                try:
                    # Create HTML file as sibling to XML file with new naming convention
