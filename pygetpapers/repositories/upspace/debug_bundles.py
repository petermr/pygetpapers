...
def debug_bundles():
    ...
    if bundles_data:
        ...
        print(
            f"✓ Bundles data keys: {list(bundles_data.keys()) if isinstance(bundles_data, dict) else 'Not a dict'}"
        )
                    ...
                    for i, bundle in enumerate(bundles):
                        ...
                        if isinstance(bundle, dict):
                            ...
                            if bundle.get("name") == "ORIGINAL":
                                ...
                                bitstreams_data = upspace._get_bundle_bitstreams(
                                    bundle["uuid"]
                                )

                                if bitstreams_data:
                                    print(
                                        f"✓ Bitstreams data type: {type(bitstreams_data)}"
                                    )
                                    print(
                                        f"✓ Bitstreams data keys: {list(bitstreams_data.keys()) if isinstance(bitstreams_data, dict) else 'Not a dict'}"
                                    )

                                    if isinstance(bitstreams_data, dict):
                                        print(f"✓ Full bitstreams data structure:")
                                        print(
                                            f"  _embedded keys: {list(bitstreams_data.get('_embedded', {}).keys())}"
                                        )
                                        print(
                                            f"  page keys: {list(bitstreams_data.get('page', {}).keys())}"
                                        )
                                        ...
                                        if (
                                            ...
                                            and "bitstreams"
                                            in bitstreams_data["_embedded"]
                                        ):
                                            bitstreams = bitstreams_data["_embedded"][
                                                "bitstreams"
                                            ]
                                            print(
                                                f"✓ Found {len(bitstreams)} bitstreams in _embedded.bitstreams"
                                            )
                                            ...
                                            for j, bitstream in enumerate(bitstreams):
                                                ...
                                                if isinstance(bitstream, dict):
                                                    print(
                                                        f"    Name: {bitstream.get('name')}"
                                                    )
                                                    print(
                                                        f"    UUID: {bitstream.get('uuid')}"
                                                    )
                                                    print(
                                                        f"    Size: {bitstream.get('sizeBytes')}"
                                                    )
                                                    print(
                                                        f"    Format: {bitstream.get('format')}"
                                                    )
                                else:
