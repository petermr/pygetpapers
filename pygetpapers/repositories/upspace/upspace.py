...
class UPSpace(RepositoryInterface):
    ...
    def _extract_metadata(self, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata["authors"] = self._extract_field(
            item_data, "dc.contributor.author", multiple=True
        )
    ...
    def _extract_field(
        self, item_data: Dict[str, Any], field_name: str, multiple: bool = False
    ) -> Any:
        ...
        while len(articles) < max_results:
            ...
            try:
                ...
                for result in search_results:
                    ...
                    if (
                        "_embedded" in result
                        and "indexableObject" in result["_embedded"]
                    ):
                            ...
                            for bitstream in bitstreams:
                                if isinstance(bitstream, dict):
                                    ...
                                    if filename.lower().endswith(".pdf"):
                                        ...
                                        if bitstream_uuid and self._download_bitstream(
                                            bitstream_uuid, pdf_path
                                        ):
    ...
    def upspace(
        self, query: str, output_dir: str, max_results: int = 20, **kwargs
    ) -> None:
    ...
    def _create_upspace_datatables_html(self, articles: List[Dict[str, Any]]) -> None:
        ...
        # Read the template
        template_path = Path(
            Path(__file__).parent, "templates", "upspace_datatables.html"
        )
        ...
        for article in articles:
            ...
            for sdg in article.get("sdg_classifications", []):
                ...
                if match:
                    sdg_num = match.group(1)
                    sdg_badges.append(
                        f'<span class="sdg-badge sdg-{sdg_num}">{sdg_num}</span>'
                    )
            ...
            if article.get("handle_url"):
                identifiers.append(
                    f'<div class="handle-url">Handle: <a href="{article["handle_url"]}" target="_blank">{article["handle_url"]}</a></div>'
                )
            if article.get("doi"):
                identifiers.append(
                    f'<div class="doi-link">DOI: <a href="https://doi.org/{article["doi"]}" target="_blank">{article["doi"]}</a></div>'
                )
            ...
            if pdf_path.exists():
                local_files.append(
                    f'<a href="{article_id}/fulltext.pdf" class="btn btn-success" target="_blank">PDF</a>'
                )

            metadata_path = Path(self.output_dir, article_id, "metadata.json")
            if metadata_path.exists():
                local_files.append(
                    f'<a href="{article_id}/metadata.json" class="btn btn-info" target="_blank">JSON</a>'
                )
    ...
    def _create_upspace_index_html(self, articles: List[Dict[str, Any]]) -> None:
        ...
        # Count SDG articles
        sdg_articles = sum(
            1 for article in articles if article.get("sdg_classifications")
        )
        ...
        html_content = html_content.replace(
            "{{DOWNLOADED_ARTICLES}}", str(len(articles))
        )
