    ...
    for pattern in search_patterns:
        try:
            ...
            if response.status_code == 200:
                ...
                if search_form:
                    print(f"Found search form: {search_form.get('action', 'No action')}")
                    print(f"Method: {search_form.get('method', 'No method')}")
                
                # Look for search input
                search_input = soup.find('input', {'type': 'search'}) or soup.find('input', {'name': re.compile(r'q|query|search')})
                if search_input:
                    print(f"Found search input: {search_input.get('name', 'No name')}")
                
                # Look for results container
                results_containers = soup.find_all(['div', 'section', 'main'], class_=re.compile(r'result|search|paper|article'))
                print(f"Found {len(results_containers)} potential results containers")
                
                # Save the HTML for inspection
                with open(f"biorxiv_search_page_{pattern.split('/')[-1]}.html", 'w', encoding='utf-8') as f:
                    f.write(response.text)
                print(f"Saved HTML to biorxiv_search_page_{pattern.split('/')[-1]}.html")
    ...
    for url in search_urls:
        try:
            ...
            if response.status_code == 200:
                ...
                paper_elements = soup.find_all(['article', 'div', 'section'], class_=re.compile(r'paper|result|entry|article'))
                print(f"Found {len(paper_elements)} potential paper elements")
                
                # Look for titles
                titles = soup.find_all(['h1', 'h2', 'h3', 'h4'], class_=re.compile(r'title'))
                ...
                # Look for pagination
                pagination = soup.find_all(['nav', 'div'], class_=re.compile(r'pagination|page'))
