...
def test_security_framework():
    ...
    try:
        ...
        # Test rate limiting
        assert resource_mgr.can_make_request("crossref.org"), "Should allow first request"
...
def test_integration():
    ...
    try:
        ...
        manager = DeclarativeOperationsManager(
            "config/crossref_declarative.yaml", 
            working_dir
        )
        
        # Test that security framework is available
        assert hasattr(manager, 'security'), "Manager should have security framework"
        assert 'validator' in manager.security, "Security framework should have validator"
        print("✅ Security framework integrated")
        
        # Test operation execution (simulation)
        for op_name, operation in list(manager.operations.items())[:2]:  # Test first 2 operations
