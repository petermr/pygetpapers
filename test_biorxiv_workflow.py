...
def demonstrate_full_workflow():
    ...
    papers_found = []
    for paper_data in scraper.search_with_pagination(query, max_papers, results_per_page=10):
        ...
        print(f"   Page: {pagination['current_page']}, Paper #{pagination['papers_found']}")
    ...
    # Get first page
    first_page_papers = []
    for paper_data in scraper.search_with_pagination(query, max_papers=2, results_per_page=10, start_page=0):
    ...
    # Get second page
    second_page_papers = []
    for paper_data in scraper.search_with_pagination(query, max_papers=2, results_per_page=10, start_page=1):
    ...
    if first_dois != second_dois:
        print(f"   ✅ Pagination working: different papers on different pages")
    else:
        print(f"   ⚠️  Pagination issue: same papers on different pages")
    ...
    summary = {
        ...
        'success_rate': len(processed_papers) / len(papers_found) if papers_found else 0,
    ...
    batch_results = scraper.batch_download_papers("climate change", max_papers=2, results_per_page=10)
    
    print(f"   Batch completed: {batch_results['summary']['total_papers_downloaded']} papers")
...
def demonstrate_cursor_reset():
    ...
    if first_papers == second_papers:
        print(f"✅ Cursor reset working: same results on repeated searches")
    else:
        print(f"⚠️  Cursor reset issue: different results on repeated searches")
...
def main():
    ...
    try:
        ...
        print(f"✅ Main workflow: SUCCESS")
        print(f"✅ Cursor reset: {'SUCCESS' if cursor_working else 'ISSUE'}")
        
        print(f"\n📁 Output directories created:")
        print(f"   - biorxiv_workflow_output/ (main workflow)")
        print(f"   - biorxiv_cursor_test/ (cursor test)")
