...
def create_config_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"✅ Created declarative configuration for {args.repository} at {args.output}")
        return 0
...
def list_operations_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for op_name, operation in manager.operations.items():
            ...
            print(f"   Inputs: {', '.join(operation.inputs) if operation.inputs else 'None'}")
            print(f"   Outputs: {', '.join(operation.outputs) if operation.outputs else 'None'}")
...
Examples:
  ...
  pygetpapers declarative validate -c config.yaml
    ...
    create_parser.add_argument("-o", "--output", required=True, help="Output configuration file")
    ...
    check_parser = subparsers.add_parser("check-deps", help="Check operation dependencies")
    check_parser.add_argument("-c", "--config", required=True, help="Configuration file")
    check_parser.add_argument("-w", "--working-dir", default=".", help="Working directory")
    ...
    execute_parser.add_argument("-c", "--config", required=True, help="Configuration file")
    execute_parser.add_argument("-w", "--working-dir", default=".", help="Working directory")
    ...
    build_parser.add_argument("-c", "--config", required=True, help="Configuration file")
    build_parser.add_argument("-w", "--working-dir", default=".", help="Working directory")
    ...
    validate_parser.add_argument("-c", "--config", required=True, help="Configuration file")
