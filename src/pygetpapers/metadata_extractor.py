...
class MetadataExtractor:
    ...
    TITLE_SELECTORS = [
        ...
        '.titulo', '.titulo-articulo', '.paper-title',
        '[ng-bind*="title"]', '[ng-bind*="titulo"]',
        '.ng-binding'
    ]
    
    AUTHOR_SELECTORS = [
        '.authors', '.author', '.author-list', 'span[class*="author"]',
        '.autores', '.autor', '.author-name', '.contrib-author'
    ]
    ...
    JOURNAL_SELECTORS = [
        '.journal', '.revista', '.publication', 'span[class*="journal"]',
        '.journal-title', '.revista-titulo', '.publication-title'
    ]
    
    KEYWORD_SELECTORS = [
        '.keywords', '.palabras-clave', '.tags', 'span[class*="keyword"]',
    ...
    def _extract_title_from_text(text_content: str) -> str:
        ...
        for pattern in title_patterns:
            ...
            for match in matches:
                ...
                if (len(title) > 10 and 
                    'redalyc' not in title.lower() and
                    'sistema' not in title.lower() and
                    'información' not in title.lower() and
                    'científica' not in title.lower()):
                    return title
        
        return ""
    
    @staticmethod
    def extract_authors(html_content: str, soup: Optional[BeautifulSoup] = None) -> List[str]:
        """
    ...
    def _extract_authors_from_text(text_content: str) -> List[str]:
        ...
        for pattern in author_patterns:
            ...
            for match in matches:
                ...
                if (len(author_text) > 5 and 
                    'redalyc' not in author_text.lower() and
                    'sistema' not in author_text.lower() and
                    'información' not in author_text.lower() and
                    'científica' not in author_text.lower() and
                    'darwiniana' not in author_text.lower()):
                    authors.append(author_text)
        
        return list(set(authors))  # Remove duplicates
    
    @staticmethod
    def extract_abstract(html_content: str, soup: Optional[BeautifulSoup] = None) -> str:
        """
    ...
    def extract_doi(html_content: str) -> str:
        ...
        for pattern in doi_patterns:
            ...
            if doi_match:
                doi = doi_match.group(1) if len(doi_match.groups()) > 0 else doi_match.group(0)
                return doi.strip()
        
        return ""
    
    @staticmethod
    def extract_language(html_content: str, soup: Optional[BeautifulSoup] = None) -> str:
        """
    ...
    @staticmethod
    def extract_keywords(html_content: str, soup: Optional[BeautifulSoup] = None) -> List[str]:
        """
            ...
            try:
                ...
                if elements:
                    ...
                    for elem in elements:
                        ...
                        if keyword_text:
                            keywords.extend([kw.strip() for kw in keyword_text.split(',') if kw.strip()])
                    if keywords:
                        return keywords
            except Exception:
                continue
    ...
    @staticmethod
    def extract_links(html_content: str, soup: Optional[BeautifulSoup] = None, base_url: str = "") -> Dict[str, str]:
        """
    ...
    def extract_all_metadata(html_content: str, url: str = "") -> Dict[str, Any]:
        ...
        """
        soup = BeautifulSoup(html_content, 'html.parser') if BEAUTIFULSOUP_AVAILABLE else None
