        ...
        for page_num, page in enumerate(pdf.pages, 1):
            ...
            if chars:
                ...
                for char in chars:
                    ...
                    if (
                        current_block
                        and abs(char.get("y0", 0) - current_block["y0"])
                        < 5  # Same line
                        and abs(char.get("x0", 0) - current_block["x1"])
                        < 20  # Close horizontally
                        and char.get("fontname")
                        == current_block["fontname"]  # Same font
                        and char.get("size") == current_block["size"]
                    ):  # Same size
            ...
            print(
                f"    Extracted {len(text_blocks)} text blocks, {len(images)} images, {len(graphics)} graphics"
            )
