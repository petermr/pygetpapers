...
from pygetpapers.core.hocr_builder import HOCRBuilder, HOCRCharacter, HOCRWord, HOCRLine, HOCRParagraph
...
def test_basic_hocr_creation():
    ...
    print(f"✅ Added image: {image['alt_text']} at ({image['x0']},{image['y0']})-({image['x1']},{image['y1']})")
    ...
    print(f"✅ Added graphic: {graphic['type']} at ({graphic['x0']},{graphic['y0']})-({graphic['x1']},{graphic['y1']})")
...
def test_advanced_features():
    ...
    for i, ptype in enumerate(paragraph_types):
        ...
        words = [
            (f"Sample {ptype}", 50, 50 + i*30, 150, 70 + i*30, 0.95, "Arial", 12 + i, "bold"),
    ...
    for text, x0, y0, x1, y1, conf, font, size, weight, style, color, baseline in char_data:
        char = builder.add_character(text, x0, y0, x1, y1, conf, font, size, weight, style, color, baseline)
        print(f"   ✅ Added character: '{char.text}' (color: {char.color}, baseline: {char.baseline})")
    
    # Add images and graphics
    image = builder.add_image(300, 50, 500, 200, alt_text="Sample figure", confidence=0.9)
    ...
    '''
    graphic = builder.add_graphic("complex", 50, 300, 250, 400, svg_data, confidence=0.85)
    print(f"✅ Added complex graphic: {graphic['type']} (confidence: {graphic['confidence']})")
