...
class TestFontDecomposition(unittest.TestCase):
    ...
    def test_pdfplumber_integration_with_font_decomposition(self):
        ...
        self.assertEqual(len(paragraph.lines), 2)  # Two lines due to y-position grouping
    ...
    def test_complex_font_patterns(self):
        ...
        for i, (font_name, expected_family, expected_weight, expected_style) in enumerate(patterns):
            word = builder.add_word_with_font_name(
                f"Word{i}", 50 + i*60, 50, 100 + i*60, 70,
                font_name=font_name,
                font_size=12
            )
